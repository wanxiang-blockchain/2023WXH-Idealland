...
contract IDCard {
    ...
    // structur for a beloved article
    struct favorite {
        bytes24 author;
        uint no;
        uint award;
    ...
    //favorites
    mapping(bytes24 => favorite[]) public favorites;
    mapping(bytes24 => uint) public favo_count;
    ...
    //modifiers
    modifier silverFee(uint amount) { require(amount == FEE[2]); _; }
    modifier copperFee(uint amount) { require(amount == FEE[3]); _; }
    modifier nameFree(bytes24 _name) { require(info[_name].used==false); _; } // ID name not used yet
    ...
    activeUser(IDs[msg.sender])
    nameUsed(aim){
        if(rStatus[IDs[msg.sender]][aim].followed==true && rStatus[IDs[msg.sender]][aim].followed==true){
            rStatus[IDs[msg.sender]][aim].active = false;
            follow_count[IDs[msg.sender]] -= 1;
            followed_count[aim] -= 1;
            //info[aim].prestige -= PRESTIGE[info[IDs[msg.sender]].level]; // if user updated then unfollow, it goes negative
        }
    }
    ...
    //share article
    function aaarticle(string _content) public
    ...
    //like an article
    function like(bytes24 aim, uint number) public payable
    activeUser(IDs[msg.sender])
    articleExist(aim, number){
        require(msg.value > 0);
        articles[aim][number].like += 1;
        wallet[info[aim].addr] += msg.value;
        favorites[IDs[msg.sender]].push(favorite({
            author : aim,
            no : number,
            award : msg.value,
            time : now
        }));
        favo_count[IDs[msg.sender]] += 1;
    }
